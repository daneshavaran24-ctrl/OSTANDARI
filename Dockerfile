...
#
# ⚠️ با مجموعه‌ی پلاگین فعلی (openai، bey، ai-coustics) این دستور **صفر بایت**
# دانلود می‌کند و فقط ۳۶ ثانیه صرف ایمپورت کردن پلاگین‌ها می‌شود — اندازه‌گیری
# شده، نه تخمین. چون بیلد لیارا مهلت زمانی دارد، پیش‌فرض خاموش است.
#
# اگر روزی پلاگینی اضافه شد که مدل محلی دارد (مثلاً turn-detector)، این را با
# `--build-arg DOWNLOAD_MODELS=1` روشن کنید؛ وگرنه آن مدل در اولین گفت‌وگوی
# واقعی دانلود می‌شود و چون کش ماندگار نیست، با هر ریستارت دوباره.
ARG DOWNLOAD_MODELS=0
RUN if [ "$DOWNLOAD_MODELS" = "1" ]; then \
      uv run --no-dev --frozen --module livekit.agents download-files; \
    else \
      echo "پیش‌دانلود مدل‌ها خاموش است (DOWNLOAD_MODELS=0)"; \
    fi
