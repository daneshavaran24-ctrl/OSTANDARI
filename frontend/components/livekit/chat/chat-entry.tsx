    ...
    >
      {(!hideTimestamp || !hideName || hasBeenEdited) && (
        <span className="text-muted-foreground flex text-sm">
          ...
          {!hideTimestamp && (
            <span className="align-self-end ms-auto font-mono text-xs opacity-0 transition-opacity ease-linear group-hover:opacity-100">
              {hasBeenEdited && '*'}
      ...
      <span className={cn('max-w-4/5 rounded-[20px] p-2', isUser ? 'bg-muted ms-auto' : 'me-auto')}>
        {message}
      </span>
    </li>
  );
